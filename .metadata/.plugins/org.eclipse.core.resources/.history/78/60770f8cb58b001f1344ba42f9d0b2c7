...
public class AddNewRoom extends JFrame {
	...
	public AddNewRoom(ManageRoom mr) {
		...
		lblNewLabel_1_1_1_1_1_1_1.setBounds(-7, 231, 90, 13);
		...
		btnAddNewRoom.setBounds(7, 304, 161, 21);
		btnAddNewRoom.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				/////////////
				String roomNo = textField.getText();
		        String nuOfBedsStr = textField_7.getText();
		        int nuOfBeds;

		        // Validate roomNo starts with "RN"
		        if (!roomNo.startsWith("RN")) {
		            // Show error message or prompt
		            JOptionPane.showMessageDialog(null, "Room number must start with 'RN'.", "Invalid Input", JOptionPane.ERROR_MESSAGE);
		            return;
		        }

		        // Validate nuOfBeds is a positive number
		        try {
		            nuOfBeds = Integer.parseInt(nuOfBedsStr);
		            if (nuOfBeds <= 0) {
		                JOptionPane.showMessageDialog(null, "Number of beds must be greater than 0.", "Invalid Input", JOptionPane.ERROR_MESSAGE);
		                return;
		            }
		        } catch (NumberFormatException ex) {
		            // Show error message if input is not a number
		            JOptionPane.showMessageDialog(null, "Number of beds must be a valid number.", "Invalid Input", JOptionPane.ERROR_MESSAGE);
		            return;
		        }
				/////////////
